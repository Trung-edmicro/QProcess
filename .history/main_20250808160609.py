...
def process_exam_content(content):
    ...
    """
    try:
        print("🔧 DEBUG: Bắt đầu xử lý exam content...")
        lines = content.split('\n')
        processed_lines = []
        ...
        # Phát hiện các phần
        is_multiple_choice = ("thí sinh trả lời từ câu 1 đến câu 12" in content_lower and 
                             "mỗi câu hỏi thí sinh chỉ chọn một phương án" in content_lower)
        is_true_false = ("thí sinh trả lời từ câu 1 đến 4" in content_lower and 
                        "trong mỗi ý a), b), c), d)" in content_lower and 
                        "thí sinh chọn đúng hoặc sai" in content_lower)
        is_essay = "thí sinh trả lời từ câu 1 đến câu 6" in content_lower
        
        print(f"🔧 DEBUG: Multiple choice detected = {is_multiple_choice}")
        print(f"🔧 DEBUG: True/false detected = {is_true_false}")
        print(f"🔧 DEBUG: Essay detected = {is_essay}")
        ...
        while i < len(lines):
            ...
            if "phần i" in line.lower():
                ...
                print(f"🔧 DEBUG: Detected section = {current_section}")
            ...
            # Xử lý câu hỏi
            if line.startswith("**Câu ") and line.endswith(":**"):
                print(f"🔧 DEBUG: Found question at line {i}: {line}")
                
                # Tìm đáp án cho câu hỏi
                answer = find_answer_for_question(lines, i, current_section)
                print(f"🔧 DEBUG: Found answer = {answer}")
                
                # Thêm template lời giải
                if current_section == "multiple_choice":
                    template = generate_multiple_choice_template(answer)
                elif current_section == "true_false":
                    template = generate_true_false_template(answer)
                elif current_section == "essay":
                    template = generate_essay_template()
                else:
                    # Tự động phát hiện dựa trên nội dung
                    print("🔧 DEBUG: Auto-detecting question type...")
                    if has_abcd_options(lines, i):
                        template = generate_multiple_choice_template(find_bold_answer(lines, i))
                        print("🔧 DEBUG: Auto-detected as multiple choice")
                    elif has_abcd_lowercase_options(lines, i):
                        template = generate_true_false_template(find_true_false_answers(lines, i))
                        print("🔧 DEBUG: Auto-detected as true/false")
                    else:
                        template = generate_essay_template()
                        print("🔧 DEBUG: Auto-detected as essay")
                
                print(f"🔧 DEBUG: Generated template = {template}")
                
                # Tìm vị trí kết thúc câu hỏi để chèn template
                end_pos = find_question_end(lines, i)
                print(f"🔧 DEBUG: Question end position = {end_pos}")
                
                if end_pos != -1:
                    # Chèn template sau câu hỏi
                    processed_lines.extend(template)
                    template_added += 1
                    print(f"🔧 DEBUG: Added template #{template_added}")
                    i = end_pos
                    continue
            
            i += 1
        
        print(f"🔧 DEBUG: Total templates added = {template_added}")
        result = '\n'.join(processed_lines)
        print(f"🔧 DEBUG: Final content length = {len(result)} chars")
        return result
...
def find_answer_for_question(lines, start_idx, section_type):
    """Tìm đáp án cho câu hỏi"""
    if section_type == "multiple_choice":
        return find_bold_answer(lines, start_idx)
    elif section_type == "true_false":
        return find_true_false_answers(lines, start_idx)
    else:
        return None

def find_bold_answer(lines, start_idx):
...
def find_true_false_answers(lines, start_idx):
...
def has_abcd_options(lines, start_idx):
...
def has_abcd_lowercase_options(lines, start_idx):
    """Kiểm tra xem câu hỏi có lựa chọn a), b), c), d) không"""
...
def find_question_end(lines, start_idx):
    """Tìm vị trí kết thúc của câu hỏi"""
    # Tìm câu hỏi tiếp theo hoặc phần tiếp theo
    for i in range(start_idx + 1, len(lines)):
        line = lines[i].strip()
        
        # Kết thúc khi gặp câu hỏi mới hoặc phần mới
        if (line.startswith("**Câu ") and line.endswith(":**")) or \
           (line.startswith("**Phần ") and "**" in line):
            return i - 1
    
    return len(lines) - 1

def generate_multiple_choice_template(answer):
...
def generate_true_false_template(answers):
    ...
    try:
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            if successful_results:
                ...
                processed_content = process_exam_content(combined_content)
...
def test_exam_processing():
    """Test function xử lý exam content với file markdown có sẵn"""
    print("=== TEST XỬ LÝ EXAM CONTENT ===")
    
    # Test với file test cụ thể
    test_file = os.path.join(app_config.output_folder, "test_original_content.md")
    
    if not os.path.exists(test_file):
        print(f"❌ Không tìm thấy file test: {test_file}")
        return
    
    print(f"📄 Đang test với file: {os.path.basename(test_file)}")
    ...
    try:
        ...
        print(f"📖 Đã đọc {len(original_content)} ký tự từ file gốc")
        print("\n=== NỘI DUNG GỐC (50 ký tự đầu) ===")
        print(original_content[:50] + "...")
        
        # Xử lý thêm template
        print("\n🔄 Đang xử lý thêm template lời giải...")
        processed_content = process_exam_content(original_content)
        
        # Lưu file mới với suffix _processed
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        base_name = os.path.splitext(os.path.basename(test_file))[0]
        new_filename = f"{base_name}_with_template_{timestamp}.md"
        new_file_path = os.path.join(app_config.output_folder, new_filename)
        
        with open(new_file_path, 'w', encoding='utf-8') as f:
            f.write(processed_content)
        
        print(f"✅ Đã lưu file đã xử lý: {new_filename}")
        print(f"📊 Kích thước file gốc: {len(original_content)} ký tự")
        print(f"📊 Kích thước file đã xử lý: {len(processed_content)} ký tự")
        print(f"📈 Tăng thêm: {len(processed_content) - len(original_content)} ký tự")
        
        # So sánh ngắn gọn
        if len(processed_content) > len(original_content):
            print("✅ Template đã được thêm vào thành công!")
            
            # Hiển thị sample kết quả
            print("\n=== SAMPLE KẾT QUẢ ĐÃ XỬ LÝ ===")
            lines = processed_content.split('\n')
            for i, line in enumerate(lines):
                print(f"{i+1:2d}: {line}")
                if i > 20:  # Chỉ hiển thị 20 dòng đầu
                    print("    ... (còn lại)")
                    break
        else:
            print("⚠️ Có thể template chưa được thêm hoặc nội dung không thay đổi")
