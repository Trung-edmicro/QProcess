            ...
            if self.vertex_config.is_configured():
                ...
                self.model = GenerativeModel(
                    ...
                    generation_config=GenerationConfig(
                        ...
                        top_p=0.8,
                        top_k=20,
                        max_output_tokens=16384  # Tăng lên để xử lý content lớn
                    )
                )
...
"""
    ...
    def _process_content_with_ai(self, content):
        ...
        """
        try:
            # Kiểm tra độ dài content và chia nhỏ nếu cần
            content_length = len(content)
            print(f"📏 Độ dài nội dung: {content_length:,} ký tự")
            
            # Nếu content quá lớn, chia thành chunks nhỏ hơn
            if content_length > 50000:  # ~50k chars
                print("⚠️ Nội dung lớn, sẽ xử lý theo chunks...")
                return self._process_large_content_in_chunks(content)
            
            # Tạo prompt tối ưu cho AI
            prompt = f"""Bạn là chuyên gia giáo dục. Nhiệm vụ: mapping câu hỏi với lời giải từ nội dung sau.

YÊU CẦU:
1. Tìm tất cả câu hỏi (Câu 1, Câu 2, ...) và lời giải tương ứng
2. Mapping chính xác mỗi câu hỏi với lời giải của nó
3. Giữ nguyên format LaTeX cho công thức toán
4. Xuất theo format chuẩn
...
HÃY BẮT ĐẦU MAPPING:"""
    ...
    def _process_large_content_in_chunks(self, content):
        """
        Xử lý content lớn bằng cách chia thành chunks nhỏ hơn
        Args:
            content: Nội dung lớn cần xử lý
        Returns:
            str: Nội dung đã được mapping
        """
        try:
            # Chia content dựa trên pattern câu hỏi
            import re
            ...
            if len(parts) <= 1:
                print("⚠️ Không thể chia nhỏ content, xử lý nguyên khối...")
                return self._process_content_with_simple_prompt(content)
            
            print(f"📋 Chia content thành {len(parts)} phần")
            ...
            for i, chunk in enumerate(parts):
                ...
                print(f"🔄 Xử lý phần {i+1}/{len(parts)}...")
                
                chunk_result = self._process_content_with_simple_prompt(chunk)
                if chunk_result:
                    results.append(chunk_result)
                else:
                    print(f"⚠️ Phần {i+1} không xử lý được, bỏ qua...")
            
            if results:
                # Kết hợp các kết quả
                final_result = "\n\n---\n\n".join(results)
                print(f"✅ Hoàn thành xử lý {len(results)} phần")
                return final_result
            else:
                print("❌ Không có phần nào được xử lý thành công")
                return None
                
        except Exception as e:
            print(f"❌ Lỗi xử lý chunks: {e}")
            return None
    
    def _process_content_with_simple_prompt(self, content):
        """
        Xử lý content với prompt đơn giản cho chunks nhỏ
        """
        try:
            prompt = f"""Mapping câu hỏi với lời giải từ nội dung sau:
