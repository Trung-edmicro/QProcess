...
"""
    ...
    def _process_content_with_ai(self, content):
        ...
        try:
            ...
            prompt = f"""
Bạn là một chuyên gia giáo dục. Nhiệm vụ của bạn là phân tích và tổ chức lại nội dung sau thành định dạng câu hỏi-đáp án được mapping rõ ràng.

NÔÏI DUNG CẦN XỬ LÝ:
{content}

YÊU CẦU:
1. Phân tích toàn bộ nội dung để tìm câu hỏi và đáp án/lời giải
2. Mapping (ghép) mỗi câu hỏi với đáp án/lời giải tương ứng
3. Tổ chức lại thành format rõ ràng

ĐỊNH DẠNG OUTPUT:
# Đề thi đã mapping câu hỏi và lời giải

*Được xử lý tự động vào {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}*
...
### ✅ Lời giải
[Nội dung lời giải/đáp án cho câu 1]
...
### ✅ Lời giải
[Nội dung lời giải/đáp án cho câu 2]

---

... (tiếp tục cho các câu khác)

HÃY XỬ LÝ VÀ TRẢ VỀ KẾT QUẢ THEO ĐÚNG FORMAT TRÊN:
"""
