...
Hỗ trợ 2 modes:
- Mode 1: Vertex AI OCR (Gemini)
- Mode 2: Mathpix OCR API
"""
import os
import sys
import time
...
def process_multiple_images_mathpix(image_paths, max_workers=None):
    """
    Xử lý nhiều ảnh đồng thời bằng multiprocessing với Mathpix API - Mode 2
    Args:
        image_paths: list đường dẫn các ảnh
        max_workers: số process tối đa (mặc định = số CPU)
    Returns:
    ...
    print(f"🚀 Bắt đầu xử lý {len(image_paths)} ảnh với Mathpix API ({max_workers} processes)")
    ...
    try:
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            # Submit tất cả tasks
            future_to_info = {
                executor.submit(process_single_image_mathpix, info): info 
                for info in image_info_list
            }
...
def save_multiple_results_to_markdown_mathpix(results, output_folder):
    ...
    try:
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            # Header
            f.write("# Kết quả OCR từ Mathpix API\n\n")
            f.write(f"**Thời gian xử lý:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            f.write(f"**Tổng số ảnh:** {len(results)}\n")
            f.write(f"**Thành công:** {len(successful_results)}\n")
            f.write(f"**Thất bại:** {len(failed_results)}\n\n")
            f.write("---\n\n")
            
            # Kết quả thành công - xử lý và thêm template
            if successful_results:
                f.write("## ✅ Nội dung OCR thành công\n\n")
...
def save_single_result_to_markdown_mathpix(result_text, image_path, output_folder):
    ...
    try:
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            # Header
            f.write("# Kết quả OCR từ Mathpix API\n\n")
            f.write(f"**File ảnh:** {os.path.basename(image_path)}\n")
            f.write(f"**Thời gian:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            f.write("---\n\n")
            
            # Nội dung OCR - xử lý với ExamProcessor
...
def single_image_mode_mathpix(image_path):
    """Test xử lý 1 ảnh đơn lẻ với Mathpix API - Mode 2"""
    print(f"\n🔄 CHẾ ĐỘ: Xử lý ảnh đơn lẻ (Mathpix API)")
    print(f"📷 Ảnh: {os.path.basename(image_path)}")
    
    result = ocr_single_image_mathpix(image_path, index=None, show_result=True)
...
def multiple_images_mode_mathpix(image_paths, max_workers=None):
    """Test xử lý nhiều ảnh đồng thời với Mathpix API - Mode 2"""
    print(f"\n🔄 CHẾ ĐỘ: Xử lý đa tiến trình (Mathpix API)")
    
    # Xử lý đa tiến trình
    results = process_multiple_images_mathpix(image_paths, max_workers)
...
def main():
    ...
    print("1️⃣  Mode 1: Vertex AI OCR (Gemini 2.5-pro)")
    print("2️⃣  Mode 2: Mathpix API OCR")
    print("3️⃣  Test Mode: Mathpix API Test")
    print("4️⃣  Test Mode: ExamProcessor Test")
    print("0️⃣  Thoát")
    ...
    print(f"\n📷 Tìm thấy {num_images} ảnh trong thư mục input:")
    ...
    if mode == 1:
        ...
        if num_images == 1:
            # Mode 2: Xử lý 1 ảnh đơn lẻ
            single_image_mode_mathpix(image_paths[0])
        else:
            # Xử lý với số process = số CPU hoặc số ảnh (tùy cái nào nhỏ hơn)
            max_workers = min(num_images, mp.cpu_count())
            print(f"🚀 Sử dụng {max_workers} processes")
            multiple_images_mode_mathpix(image_paths, max_workers)

def test_mathpix_mode():
    """Test Mathpix mode với file có sẵn - tích hợp từ test_mathpix_mode.py"""
    
    print("🧪 TEST MATHPIX MODE 2")
    print("=" * 50)
    ...
    test_image = "data/input/testOCR4.png"
    
    if os.path.exists(test_image):
        print(f"\n📄 TEST SINGLE IMAGE: {os.path.basename(test_image)}")
        print("-" * 40)
        
        result = ocr_single_image_mathpix(test_image, show_result=True)

        if result and result[1]:  # success
            print("✅ Single image test thành công!")
            
            # Sử dụng function save riêng cho single result
            output_file = save_single_result_to_markdown_mathpix(
                result[0],  # result_text
            ...
            print("❌ Single image test thất bại!")
            if result:
                print(f"   Lỗi: {result[2]}")  # error_msg
    else:
        print(f"⚠️ Test image không tồn tại: {test_image}")
    
    # Test với nhiều ảnh nếu có
    test_images = [
        "data/input/testOCR1.png",
        "data/input/testOCR2.png", 
        "data/input/testOCR3.png"
    ]
    
    existing_images = [img for img in test_images if os.path.exists(img)]
    
    if len(existing_images) > 1:
        print(f"\n📄 TEST MULTIPLE IMAGES: {len(existing_images)} ảnh")
        print("-" * 40)
        
        results = process_multiple_images_mathpix(existing_images, max_workers=2)
        
        if results:
            print("✅ Multiple images test thành công!")
            ...
            print("❌ Multiple images test thất bại!")
    else:
        print("⚠️ Không đủ ảnh để test multiple mode")
    
    print("\n🎉 Test hoàn thành!")

def test_exam_processor():
    """Test function ExamProcessor - tích hợp từ test_processor.py"""
    
    print("🧪 TEST EXAM PROCESSOR")
    print("=" * 50)
    
    # Sử dụng file gốc chưa được xử lý
    output_folder = "data/output"
    target_file = os.path.join(output_folder, "ocr_multiple_results.md")
    
    if not os.path.exists(target_file):
        print("❌ Không tìm thấy file ocr_multiple_results.md trong thư mục data/output")
        print("💡 Hãy chạy OCR trước để tạo file này")
        
        # Tự động tạo sample data để test
        print("\n🔄 Tạo sample data để test...")
        sample_content = """**Phần I. TRẮC NGHIỆM (7,0 điểm)**

**Câu 1:** Trong không gian với hệ tọa độ Oxyz, cho mặt phẳng (P): 2x - y + 3z - 6 = 0. Điểm nào sau đây thuộc mặt phẳng (P)?
A. M(1, 2, 3)  
B. N(0, 0, 2)  
C. P(1, 1, 1)  
D. Q(2, 1, 0)

**Câu 2:** Tính tích phân $\\int_0^1 x^2 dx$
A. $\\frac{1}{3}$  
B. $\\frac{1}{2}$  
C. $\\frac{2}{3}$  
D. $1$
...
**Câu 3:** Cho hình chóp S.ABC có đáy ABC là tam giác vuông tại A với AB = 3, AC = 4. Biết SA vuông góc với mặt phẳng (ABC) và SA = 5. Tính thể tích khối chóp S.ABC."""
        
        try:
            os.makedirs(output_folder, exist_ok=True)
            with open(target_file, 'w', encoding='utf-8') as f:
                f.write(sample_content)
            print(f"✅ Đã tạo sample file: {target_file}")
        except Exception as e:
            print(f"❌ Lỗi khi tạo sample file: {e}")
            return
    
    print(f"📄 Đang test với file: {os.path.basename(target_file)}")
    
    try:
        # Đọc nội dung file OCR gốc
        with open(target_file, 'r', encoding='utf-8') as f:
            test_content = f.read()
        
        print(f"📖 Đã đọc {len(test_content)} ký tự từ file OCR")
        print("📝 Nội dung đầu file:")
        print("─" * 50)
        print(test_content[:300] + "...")
        print("─" * 50)
        
        print("\n🔄 Đang xử lý với ExamProcessor...")
        
        # Gọi ExamProcessor
        processed_content = ExamProcessor.process_exam_content(test_content)
        
        print(f"\n✅ Xử lý hoàn thành!")
        print(f"📊 Kích thước gốc: {len(test_content)} ký tự")
        print(f"📊 Kích thước sau xử lý: {len(processed_content)} ký tự")
        print(f"📈 Tăng thêm: {len(processed_content) - len(test_content)} ký tự")
        
        if len(processed_content) > len(test_content):
            print("🎉 Template đã được thêm thành công!")
            ...
            for i, line in enumerate(lines):
                print(f"{i+1:2d}: {line}")
                if i > 25:  # Hiển thị 25 dòng đầu
                    print("    ... (còn lại)")
                    break
                    
            # Lưu vào file để kiểm tra chi tiết
            if not os.path.exists(output_folder):
                os.makedirs(output_folder)
                
            output_file = os.path.join(output_folder, "test_processor_result.md")
            with open(output_file, 'w', encoding='utf-8') as f:
                f.write(processed_content)
            print(f"\n💾 Đã lưu kết quả chi tiết vào: {output_file}")
            
        else:
            print("⚠️ Template có thể chưa được thêm hoặc có lỗi!")
