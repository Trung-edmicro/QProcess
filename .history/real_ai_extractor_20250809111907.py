...
class RealAIGuidedExtractor:
    ...
    def analyze_image_with_ai(self, image_path):
        ...
        # Lấy kích thước ảnh để cung cấp context cho AI
        image = cv2.imread(image_path)
        if image is None:
            return self._fallback_analysis()
        
        height, width = image.shape[:2]
        print(f"📏 Cung cấp context cho AI: Image size {width}x{height}")
        
        # Tạo prompt chi tiết cho AI với context về kích thước
        prompt = f"""
You are a precise detector of NON-TEXT visual elements in exam/scanned pages.
Identify ONLY diagrams/charts/mathematical figures/geometric drawings (line drawings).
Return pixel-accurate bounding boxes.
...
DETECTION RULES
- Include the whole diagram even if composed of dashed/solid lines, arrows, or internal labels that are INSIDE the figure.
- Exclude: headers, question numbers/letters, paragraphs, pure text blocks, page margins, stamps/watermarks.
- If a diagram is fragmented (multiple pieces that clearly belong together), MERGE them into one detection.
- If two diagrams are distinct, keep them separate even if they are close. Use a gap rule: if background-only gap ≥ 12 px, treat as separate.

BOUNDING BOX REQUIREMENTS (CRITICAL)
Return TWO boxes per item:
1) "tight_bbox": [x, y, w, h] with **no padding**. Snap each side to the outermost visible stroke/edge of the diagram.
   - Left/Top = floor(outermost-ink coordinate)
   - Right/Bottom = ceil(outermost-ink coordinate)
   - Tolerance ≤ 2 px on each side.
2) "padded_bbox": [x, y, w, h] with small padding p around the tight box:
   - p = round(0.01 * max({width}, {height})) clamped to [4, 12] px
   - Must stay within image bounds

SNAPPING GUIDANCE
- Treat any visible stroke (solid or dashed) and vertex markers as “ink”.
- Ignore faint background text bleed-through behind the diagram unless it overlaps the figure boundary.
- Do NOT include neighboring question text or labels that sit clearly outside the figure frame.
- If the figure has a thin border/box, include the border fully.

OUTPUT CONFIDENCE & QA
- confidence ∈ [0.0, 1.0]
- For each item include "qa": brief note on why the box is correct (e.g., “snapped to triangle edges; 6–8 px gap to text; merged dashed base lines”).
- Provide "area_pct" (percentage of page area covered by padded_bbox, rounded to 0.1%).
  ...
  "diagrams": [
    {
      ...
      "type": "geometry|chart|diagram|figure",
      "tight_bbox": [x, y, w, h],
      "padded_bbox": [x, y, w, h],
      "confidence": 0.0–1.0,
      "area_pct": 0.0,
      "qa": "why this bbox is correct"
    }
  ],
  "total_found": <int>
}
...
- w > 0 and h > 0
- padded_bbox must fully contain tight_bbox
- total_found equals number of items in "diagrams"
        """
    ...
    def _validate_and_fix_bbox(self, bbox, img_width, img_height, diagram_id):
        """Validate và fix bbox coordinates"""
        if len(bbox) != 4:
            print(f"⚠️ Invalid bbox format for diagram {diagram_id}: {bbox}")
            return None
            
        x, y, w_box, h_box = bbox
        
        # Convert to int nếu cần
        try:
            x, y, w_box, h_box = int(x), int(y), int(w_box), int(h_box)
        except (ValueError, TypeError):
            print(f"⚠️ Non-numeric bbox for diagram {diagram_id}: {bbox}")
            return None
        
        print(f"🔍 Original bbox {diagram_id}: [{x}, {y}, {w_box}, {h_box}]")
        
        # Fix coordinates
        x_orig, y_orig, w_orig, h_orig = x, y, w_box, h_box
        ...
        # Fix width/height
        w_box = min(w_box, img_width - x)
        h_box = min(h_box, img_height - y)
        
        # Ensure positive dimensions
        w_box = max(1, w_box)
        h_box = max(1, h_box)
        
        fixed_bbox = [x, y, w_box, h_box]
        
        if [x_orig, y_orig, w_orig, h_orig] != fixed_bbox:
            print(f"🔧 Fixed bbox {diagram_id}: {bbox} → {fixed_bbox}")
        else:
            print(f"✅ Valid bbox {diagram_id}: {fixed_bbox}")
    ...
    def extract_diagrams_with_real_ai(self, image_path):
        ...
        for i, diagram in enumerate(diagrams, 1):
            ...
            # Validate và fix bbox using new function
            fixed_bbox = self._validate_and_fix_bbox(bbox, w, h, i)
            if fixed_bbox is None:
                continue
                
            x, y, w_box, h_box = fixed_bbox
    ...
    def _opencv_fallback(self, image_path):
        ...
        # Sử dụng parameters strict cho high accuracy
