...
def process_single_image(image_info):
    "        print("\n🎉 TEST THÀNH CÔNG!")
        ...
        mode = 3  # 1=đơn lẻ, 2=nhiều ảnh cụ thể, 3=tất cả ảnh trong folder
        
        if mode == 1:
            # Chế độ xử lý đơn lẻ (legacy)
            output_file = save_ocr_result_to_markdown(result, image_path, app_config.output_folder)
            
            if output_file:
                print(f"💾 Đã lưu kết quả vào file Markdown: {output_file}")
                print(f"📁 Thư mục output: {app_config.output_folder}")
                print(f"📄 Tên file: {os.path.basename(output_file)}")
            else:
                print("⚠️ Không thể lưu file markdown")
        elif mode == 3:
            # Chuyển sang chế độ xử lý tất cả ảnh
            print("🔄 Chuyển sang chế độ xử lý tất cả ảnh...")
            test_all_images_in_folder_mode()
            return  # Thoát khỏi function main
    else:
        print("\n❌ TEST THẤT BẠI!")
        
        # Vẫn có thể thử chế độ đa tiến trình
        print("🔄 Thử chế độ xử lý tất cả ảnh trong thư mục...")
        test_all_images_in_folder_mode()ột ảnh đơn lẻ - được sử dụng trong multiprocessing
    Args:
        image_info: tuple (index, image_path)
    Returns:
        tuple (index, result_text, image_path, success)
    """
...
def main():
    ...
    # Đường dẫn ảnh test
    image_path = os.path.join(app_config.input_folder, "testOCR2.png")
...
def test_single_image_mode(image_path):
...
def test_multiple_images_mode(image_paths, max_workers=None):
...
def test_all_images_in_folder_mode():
    """Test xử lý tất cả ảnh trong thư mục input"""
    print(f"\n🔄 CHẾ ĐỘ: Xử lý tất cả ảnh trong thư mục")
    print(f"📁 Thư mục: {app_config.input_folder}")
    ...
    # Xử lý với số process = số CPU
    max_workers = mp.cpu_count()
    print(f"🚀 Sử dụng {max_workers} processes")
    
    test_multiple_images_mode(image_paths, max_workers)
