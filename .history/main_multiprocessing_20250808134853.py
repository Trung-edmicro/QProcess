...
def main():
    """Hàm main để test - hỗ trợ cả xử lý đơn lẻ và đa tiến trình"""
    ...
    # Lựa chọn chế độ xử lý
    print("🔧 CHỌN CHẾ ĐỘ XỬ LÝ:")
    print("1. Xử lý 1 ảnh đơn lẻ")
    print("2. Xử lý nhiều ảnh đồng thời (multiprocessing)")
    print("3. Xử lý tất cả ảnh trong thư mục input")
    
    # Tự động chọn chế độ (có thể thay đổi logic này)
    mode = 3  # Mặc định xử lý tất cả ảnh trong thư mục
    
    if mode == 1:
        # Chế độ xử lý đơn lẻ
        image_path = os.path.join(app_config.input_folder, "testOCR2.png")
        test_single_image_mode(image_path)
        
    elif mode == 2:
        # Chế độ xử lý nhiều ảnh cụ thể
        image_paths = [
            os.path.join(app_config.input_folder, "testOCR.png"),
            os.path.join(app_config.input_folder, "testOCR2.png"),
            # Thêm các ảnh khác ở đây
        ]
        # Lọc ra các file tồn tại
        existing_paths = [path for path in image_paths if os.path.exists(path)]
        test_multiple_images_mode(existing_paths)
