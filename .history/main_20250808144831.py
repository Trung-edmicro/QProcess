...
def ocr_single_image(image_path, index=None, show_result=False):
    ...
    try:
        ...
        # Import config (cần thiết cho multiprocessing)
        from config import app_config
        ...
        if not app_config.vertex_ai.is_configured():
            error_msg = "Vertex AI chưa được cấu hình đúng!"
            if index is not None:
                return (index, None, image_path, False, error_msg)
            else:
                print(f"❌ {error_msg}")
...
def ocr_image(image_path):
    """
    Wrapper cho single image mode - gọi ocr_single_image với hiển thị kết quả
    Args:
        image_path: đường dẫn ảnh
    Returns:
        result_text hoặc None nếu thất bại
    """
    result_text, success, error_msg = ocr_single_image(image_path, index=None, show_result=True)
