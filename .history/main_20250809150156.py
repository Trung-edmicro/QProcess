...
from processors.crop_image import save_diagrams_from_line_data
...
def ocr_single_image_mathpix(image_path, index=None, show_result=False):
    ...
    try:
        ...
        mathpix_options = {
            ...
            "include_line_data": True,         # ✨ Quan trọng
            "include_diagram": True,           # ✨ Thử bật để ép bắt hình
            "include_diagram_text": True,      # ✨ Nếu muốn text trong hình
            "rm_spaces": True,
            "rm_fonts": False,
        ...
        diagram_files = save_diagrams_from_line_data(image_path, result, base_outdir="diagrams")
        if diagram_files:
        ...
        if result and result.get('text'):
            ...
            if index is not None:
                print(f"✅ {prefix} Hoàn thành: {os.path.basename(image_path)}")
                return (index, processed_text, image_path, True, None)
            else:
    ...
    if result and result[1]:  # result[1] là success flag
        ...
        try:
            ...
            with open(output_file, "w", encoding="utf-8") as f:
                ...
                f.write(result[0])  # result[0] là result_text
