...
class RealAIGuidedExtractor:
    ...
    def analyze_image_with_ai(self, image_path):
        ...
        prompt = f"""
TASK: Analyze this image and identify ONLY diagrams, charts, mathematical figures, geometric shapes, or visual elements that are NOT text.

IMAGE CONTEXT:
- Image dimensions: {width} pixels wide × {height} pixels tall
- Coordinate system: Top-left corner is (0,0), x increases rightward, y increases downward

REQUIREMENTS FOR EACH DIAGRAM/FIGURE:
1. Brief description of what the diagram shows
2. Type: geometry, chart, diagram, figure, etc.
3. PRECISE bounding box as [x, y, width, height] where:
   - x, y: top-left corner coordinates (must be ≥ 0)
   - width: horizontal span (x + width must be ≤ {width})
   - height: vertical span (y + height must be ≤ {height})
4. Confidence score (0.0 to 1.0)

CRITICAL INSTRUCTIONS:
- Examine the image CAREFULLY to determine exact boundaries
- Measure coordinates precisely relative to the {width}×{height} image
- Include some padding around diagrams but stay within bounds
- EXCLUDE pure text areas, headers, question numbers
- ONLY include actual visual diagrams/figures/charts
...
VALIDATION: Ensure ALL coordinates satisfy:
- x ≥ 0 and x < {width}
- y ≥ 0 and y < {height}  
- x + width ≤ {width}
- y + height ≤ {height}
    ...
    def extract_diagrams_with_real_ai(self, image_path):
        ...
        for i, diagram in enumerate(diagrams, 1):
            ...
            # Fix bbox nếu out of bounds
            x = max(0, min(x, w - 1))
            y = max(0, min(y, h - 1))
            w_box = min(w_box, w - x)
            h_box = min(h_box, h - y)
            
            # Validate bbox after fixing
            if w_box <= 0 or h_box <= 0:
                print(f"⚠️ Invalid bbox after fixing for diagram {i}: [{x}, {y}, {w_box}, {h_box}]")
                continue
            
            if x + w_box > w or y + h_box > h:
                print(f"⚠️ Bbox still out of bounds for diagram {i}: [{x}, {y}, {w_box}, {h_box}] (image: {w}x{h})")
                continue
