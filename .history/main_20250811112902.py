...
def ocr_single_pdf_vertex_ai(pdf_path, index=None, show_result=False):
    ...
    try:
        ...
        # Xử lý từng trang bằng Vertex AI
        if index is None:
            print(f"🔄 Xử lý {len(temp_image_paths)} trang bằng Vertex AI...")
        ...
        for i, image_path in enumerate(temp_image_paths):
            ...
            if index is None:
                print(f"📄 Đang xử lý trang {page_num}/{len(temp_image_paths)}")
            
            # Gọi OCR cho từng trang
...
def multiple_pdfs_mode_vertex_ai(pdf_paths, max_workers):
    """Xử lý nhiều PDF với Vertex AI"""
    print(f"\n📄 MULTIPLE PDFs MODE (VERTEX AI)")
    print(f"📁 {len(pdf_paths)} PDFs | 🚀 {max_workers} workers")
...
def main():
    ...
    if PDF_SUPPORT:
        print("1️⃣  Mode 1: Gemini OCR (Ảnh + PDF)")
    else:
        print("1️⃣  Mode 1: Gemini OCR (Chỉ ảnh)")
    print("2️⃣  Mode 2: Mathpix (Ảnh + PDF)")
    print("3️⃣  Test Mathpix Mode")
    print("4️⃣  Test ExamProcessor")
    print("0️⃣  Thoát")
...
def test_mathpix_mode():
    ...
    # Test với file PDF nếu có
    test_pdf = "data/input/test.pdf"
    
    if os.path.exists(test_pdf):
        print(f"\n📄 TEST SINGLE PDF: {os.path.basename(test_pdf)}")
        print("-" * 40)
        
        result = ocr_single_pdf_mathpix(test_pdf, show_result=True)

        if result and result[1]:  # success
            print("✅ Single PDF test thành công!")
            ...
            print("❌ Single PDF test thất bại!")
            if result:
                print(f"   Lỗi: {result[2]}")  # error_msg
    else:
        print(f"⚠️ Test PDF không tồn tại: {test_pdf}")
    
    # Test với nhiều file nếu có
    test_files = [
    ...
    if len(existing_files) > 1:
        print(f"\n📄 TEST MULTIPLE FILES: {len(existing_files)} file")
        print("-" * 40)
        
        results = process_multiple_files_mathpix(existing_files, max_workers=2)
        
        if results:
            print("✅ Multiple files test thành công!")
            ...
            print("❌ Multiple files test thất bại!")
    else:
        print("⚠️ Không đủ file để test multiple mode")
