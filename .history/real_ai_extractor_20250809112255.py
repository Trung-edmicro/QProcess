...
DETECTION RULES
- Include full diagram (solid/dashed lines, arrows, internal labels inside the figure).
- Exclude headers, question numbers/letters, paragraphs, pure text blocks, page margins, stamps/watermarks.
- Merge fragmented parts that clearly belong to the same diagram.
- Keep distinct diagrams separate (background-only gap ≥ 12 px → separate).

BOUNDING BOX REQUIREMENTS
Return TWO boxes per item:
1) "tight_bbox": [x, y, w, h] with no padding; snap to outermost visible stroke.
2) "padded_bbox": [x, y, w, h] with p = round(0.01*max({width},{height})) clamped to [4,12] px; must stay within bounds.
